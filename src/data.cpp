...
TokenDataset::TokenDataset(std::vector<std::int32_t> token_ids) : tokens_(std::move(token_ids)) {
  if (tokens_.size() < 1024) {
    throw std::runtime_error("token dataset too small (need at least 1024 tokens)");
  }
}

std::size_t TokenDataset::size() const {
  return tokens_.size();
}

Batch TokenDataset::sample_batch(int B, int T, util::Rng& rng) const {
  if (B <= 0 || T <= 0) throw std::runtime_error("sample_batch: invalid B/T");
  if (tokens_.size() < static_cast<std::size_t>(T + 1)) throw std::runtime_error("sample_batch: dataset too small");

  const std::int32_t max_start = static_cast<std::int32_t>(tokens_.size() - static_cast<std::size_t>(T + 1));
  ...
  for (int b = 0; b < B; ++b) {
    const std::int32_t start = rng.uniform_int(0, max_start);
    for (int t = 0; t < T; ++t) {
      batch.x[static_cast<std::size_t>(b) * T + t] = tokens_[static_cast<std::size_t>(start + t)];
      batch.y[static_cast<std::size_t>(b) * T + t] = tokens_[static_cast<std::size_t>(start + t + 1)];
    }
  }
